...
## Overview
A minimalist CLI tool that performs automated code reviews using AI models. The tool analyzes Git branch differences and generates comprehensive review reports in Markdown format.
...
The tool always reviews the currently checked out branch against the target branch.
...
- Architecture: Designed for easy extensibility to support other AI providers with minimal code changes
...
1. **read_file_part**: Read specific sections of files (with line numbers) to reduce token usage
2. **diff_file**: Show Git diff for a specific file (supports partial diffs to reduce tokens)
...
**Note**: The list of changed files is provided directly in the agent's context at initialization, eliminating the need for a separate tool call.
